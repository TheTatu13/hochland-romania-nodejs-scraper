...
function parseListing(html, selectors = scraperConfig.selectors) {
  ...
  // match the same block twice (same title AND same URL), but Hochland is
  ...
  for (let i = 0; i < scopes.length; i++) {
    ...
    // Hochland-specific: the careers page tags each job_item with
    // data-locatie="bucuresti|sovata|sighisoara|vanzari" -- this is ground
    // truth for location and should be preferred over guessing from the
    // title (which never mentions a city). "vanzari" ("Sales") is a
    // department tab, not a real city, so it intentionally maps to nothing
    // and falls back to locationFromTitle/defaultLocation downstream.
    const locatieMatch = scope.raw().match(/data-locatie=["']([a-z]+)["']/i);
    const locatie = locatieMatch ? locatieMatch[1].toLowerCase() : null;

    items.push({ title: cleaned, expirationdate: deadline, url, locatie });
  }
...
// Hochland-specific: maps the careers page's data-locatie tab codes to real
// city names. "vanzari" ("Sales") is a department tab, not a city, so it's
// intentionally left unmapped -- those jobs fall through to locationFromTitle.
const LOCATIE_TO_CITY = {
  bucuresti: "Bucuresti",
  sovata: "Sovata",
