...
|---|---|
| `HOCHLAND ROMANIA SRL` | legal name, uppercase (e.g. `EXAMPLE COMPANY SRL`) |
| `Hochland` | commercial brand |
| `10666988` | fiscal code (CUI), no `RO` prefix |
| `https://hochland.ro` | `https://www.example.com` |
| `https://hochland.ro/cariere/` | the open-positions listing page |
| `https://hochland.ro/sitemap_index.xml` | the job sitemap URL (or `""` if the site has none) |
| `https://hochland.ro/cariere/` | canonical job-permalink prefix, e.g. `https://www.example.com/jobs/` |
| `Bucuresti` | HQ city (falls back to `România` in the transform) |
| `.joburi_locatie_list .job_item` / `h4` / `` | primary CSS selectors for the listing (keep the generic fallbacks after them) |
| `TheTatu13` / `hochland-romania-nodejs-scraper` | the derived repo's owner / name |
